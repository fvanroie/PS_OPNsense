...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseAcmeClient";
        module = "acmeclient";
        controller = "validations"
    }
    switch ("Disable-OPNsenseAcmeClient".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    switch ("acmeclient") {
        { @("AcmeClient", "Freeradius", "IDS", "Monit", "Postfix", "ProxyUserACL", "Quagga", "Routes", "Siproxd", "Tinc", "Tor", "Zerotier") -Contains $_ } {           
            $objects.add('command',"search")
        }
        default {           
            $objects.add('command',"searchs")
        }
    }

    Describe -Tag "acmeclient" "Disable-OPNsenseAcmeClient" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseAcmeClient -Uuid $uuid } | should Not Throw
            }
