...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseMonit";
        module = "monit";
        controller = "settings"
    }
    switch ("Disable-OPNsenseMonit".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "monit" "Disable-OPNsenseMonit" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseMonit -Uuid $uuid } | should Not Throw
            }
