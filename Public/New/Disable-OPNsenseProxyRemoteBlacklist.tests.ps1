...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseProxyRemoteBlacklist";
        module = "proxy";
        controller = "settings"
    }
    switch ("Disable-OPNsenseProxyRemoteBlacklist".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "proxy" "Disable-OPNsenseProxyRemoteBlacklist" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseProxyRemoteBlacklist -Uuid $uuid } | should Not Throw
            }
