...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseBgpNeighbor";
        module = "quagga";
        controller = "bgp"
    }
    switch ("Disable-OPNsenseBgpNeighbor".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "quagga" "Disable-OPNsenseBgpNeighbor" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseBgpNeighbor -Uuid $uuid } | should Not Throw
            }
