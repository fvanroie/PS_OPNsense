...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Enable-OPNsenseFreeradiusUser";
        module = "freeradius";
        controller = "user"
    }
    switch ("Enable-OPNsenseFreeradiusUser".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "freeradius" "Enable-OPNsenseFreeradiusUser" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Enable-OPNsenseFreeradiusUser -Uuid $uuid } | should Not Throw
            }
