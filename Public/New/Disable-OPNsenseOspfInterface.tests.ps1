...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseOspfInterface";
        module = "quagga";
        controller = "ospfsettings"
    }
    switch ("Disable-OPNsenseOspfInterface".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "quagga" "Disable-OPNsenseOspfInterface" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseOspfInterface -Uuid $uuid } | should Not Throw
            }
