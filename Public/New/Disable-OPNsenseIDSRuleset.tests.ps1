...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseIDSRuleset";
        module = "ids";
        controller = "settings"
    }
    switch ("Disable-OPNsenseIDSRuleset".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "ids" "Disable-OPNsenseIDSRuleset" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseIDSRuleset -Uuid $uuid } | should Not Throw
            }
