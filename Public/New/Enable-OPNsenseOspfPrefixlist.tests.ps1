...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Enable-OPNsenseOspfPrefixlist";
        module = "quagga";
        controller = "ospfsettings"
    }
    switch ("Enable-OPNsenseOspfPrefixlist".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    switch ("quagga") {
        { @("AcmeClient", "Freeradius", "IDS", "Monit", "Postfix", "ProxyUserACL", "Quagga", "Routes", "Siproxd", "Tinc", "Tor", "Zerotier") -Contains $_ } {           
            $objects.add('command',"searchprefixlist")
        }
        default {           
            $objects.add('command',"searchprefixlists")
        }
    }

    Describe -Tag "quagga" "Enable-OPNsenseOspfPrefixlist" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Enable-OPNsenseOspfPrefixlist -Uuid $uuid } | should Not Throw
            }
