...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Enable-OPNsenseMonit";
        module = "monit";
        controller = "settings"
    }
    switch ("Enable-OPNsenseMonit".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "monit" "Enable-OPNsenseMonit" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Enable-OPNsenseMonit -Uuid $uuid } | should Not Throw
            }
