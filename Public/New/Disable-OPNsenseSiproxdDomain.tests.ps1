...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseSiproxdDomain";
        module = "siproxd";
        controller = "domain"
    }
    switch ("Disable-OPNsenseSiproxdDomain".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "siproxd" "Disable-OPNsenseSiproxdDomain" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseSiproxdDomain -Uuid $uuid } | should Not Throw
            }
