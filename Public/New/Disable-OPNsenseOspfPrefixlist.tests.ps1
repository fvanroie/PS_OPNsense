...
InModuleScope PS_OPNsense {
    ...
    switch ("Disable-OPNsenseOspfPrefixlist".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "quagga" "Disable-OPNsenseOspfPrefixlist" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseOspfPrefixlist -Uuid $uuid } | should Not Throw
            }
