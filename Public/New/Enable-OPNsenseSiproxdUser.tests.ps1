...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Enable-OPNsenseSiproxdUser";
        module = "siproxd";
        controller = "user"
    }
    switch ("Enable-OPNsenseSiproxdUser".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "siproxd" "Enable-OPNsenseSiproxdUser" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Enable-OPNsenseSiproxdUser -Uuid $uuid } | should Not Throw
            }
