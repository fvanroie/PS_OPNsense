...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Enable-OPNsenseTorExitacl";
        module = "tor";
        controller = "exitacl"
    }
    switch ("Enable-OPNsenseTorExitacl".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "tor" "Enable-OPNsenseTorExitacl" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Enable-OPNsenseTorExitacl -Uuid $uuid } | should Not Throw
            }
