...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseTorHiddenservice";
        module = "tor";
        controller = "hiddenservice"
    }
    switch ("Disable-OPNsenseTorHiddenservice".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "tor" "Disable-OPNsenseTorHiddenservice" {
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseTorHiddenservice -Uuid $uuid } | should Not Throw
            }
