...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseTorHiddenserviceacl";
        module = "tor";
        controller = "hiddenserviceacl"
    }
    switch ("Disable-OPNsenseTorHiddenserviceacl".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    switch ("tor") {
        { @("AcmeClient", "Freeradius", "IDS", "Monit", "Postfix", "ProxyUserACL", "Quagga", "Routes", "Siproxd", "Tinc", "Tor", "Zerotier") -Contains $_ } {           
            $objects.add('command',"searchacl")
        }
        default {           
            $objects.add('command',"searchacls")
        }
    }

    Describe -Tag "tor" "Disable-OPNsenseTorHiddenserviceacl" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseTorHiddenserviceacl -Uuid $uuid } | should Not Throw
            }
