...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsensePostfixSender";
        module = "postfix";
        controller = "sender"
    }
    switch ("Disable-OPNsensePostfixSender".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    switch ("postfix") {
        { @("AcmeClient", "Freeradius", "IDS", "Monit", "Postfix", "ProxyUserACL", "Quagga", "Routes", "Siproxd", "Tinc", "Tor", "Zerotier") -Contains $_ } {           
            $objects.add('command',"searchsender")
        }
        default {           
            $objects.add('command',"searchsenders")
        }
    }

    Describe -Tag "postfix" "Disable-OPNsensePostfixSender" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsensePostfixSender -Uuid $uuid } | should Not Throw
            }
