...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Enable-OPNsenseAcmeClient";
        module = "acmeclient";
        controller = "validations"
    }
    switch ("Enable-OPNsenseAcmeClient".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "acmeclient" "Enable-OPNsenseAcmeClient" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Enable-OPNsenseAcmeClient -Uuid $uuid } | should Not Throw
            }
