...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Enable-OPNsenseCaptivePortalZone";
        module = "captiveportal";
        controller = "settings"
    }
    switch ("Enable-OPNsenseCaptivePortalZone".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "captiveportal" "Enable-OPNsenseCaptivePortalZone" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Enable-OPNsenseCaptivePortalZone -Uuid $uuid } | should Not Throw
            }
