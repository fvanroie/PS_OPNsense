...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsensePostfixDomain";
        module = "postfix";
        controller = "domain"
    }
    switch ("Disable-OPNsensePostfixDomain".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "postfix" "Disable-OPNsensePostfixDomain" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsensePostfixDomain -Uuid $uuid } | should Not Throw
            }
