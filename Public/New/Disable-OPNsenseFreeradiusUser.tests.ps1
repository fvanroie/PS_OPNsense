...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseFreeradiusUser";
        module = "freeradius";
        controller = "user"
    }
    switch ("Disable-OPNsenseFreeradiusUser".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    switch ("freeradius") {
        { @("AcmeClient", "Freeradius", "IDS", "Monit", "Postfix", "ProxyUserACL", "Quagga", "Routes", "Siproxd", "Tinc", "Tor", "Zerotier") -Contains $_ } {           
            $objects.add('command',"searchuser")
        }
        default {           
            $objects.add('command',"searchusers")
        }
    }

    Describe -Tag "freeradius" "Disable-OPNsenseFreeradiusUser" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseFreeradiusUser -Uuid $uuid } | should Not Throw
            }
