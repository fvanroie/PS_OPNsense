...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseHAProxyLuaScript";
        module = "haproxy";
        controller = "settings"
    }
    switch ("Disable-OPNsenseHAProxyLuaScript".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "haproxy" "Disable-OPNsenseHAProxyLuaScript" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseHAProxyLuaScript -Uuid $uuid } | should Not Throw
            }
