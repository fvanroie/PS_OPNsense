...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseHAProxyFrontend";
        module = "haproxy";
        controller = "settings"
    }
    switch ("Disable-OPNsenseHAProxyFrontend".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "haproxy" "Disable-OPNsenseHAProxyFrontend" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseHAProxyFrontend -Uuid $uuid } | should Not Throw
            }
