...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseFreeradiusClient";
        module = "freeradius";
        controller = "client"
    }
    switch ("Disable-OPNsenseFreeradiusClient".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    switch ("freeradius") {
        { @("AcmeClient", "Freeradius", "IDS", "Monit", "Postfix", "ProxyUserACL", "Quagga", "Routes", "Siproxd", "Tinc", "Tor", "Zerotier") -Contains $_ } {           
            $objects.add('command',"searchclient")
        }
        default {           
            $objects.add('command',"searchclients")
        }
    }

    Describe -Tag "freeradius" "Disable-OPNsenseFreeradiusClient" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseFreeradiusClient -Uuid $uuid } | should Not Throw
            }
