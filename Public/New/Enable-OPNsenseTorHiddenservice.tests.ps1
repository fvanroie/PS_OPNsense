...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Enable-OPNsenseTorHiddenservice";
        module = "tor";
        controller = "hiddenservice"
    }
    switch ("Enable-OPNsenseTorHiddenservice".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    switch ("tor") {
        { @("AcmeClient", "Freeradius", "IDS", "Monit", "Postfix", "ProxyUserACL", "Quagga", "Routes", "Siproxd", "Tinc", "Tor", "Zerotier") -Contains $_ } {           
            $objects.add('command',"searchservice")
        }
        default {           
            $objects.add('command',"searchservices")
        }
    }

    Describe -Tag "tor" "Enable-OPNsenseTorHiddenservice" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Enable-OPNsenseTorHiddenservice -Uuid $uuid } | should Not Throw
            }
