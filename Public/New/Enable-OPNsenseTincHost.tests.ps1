...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Enable-OPNsenseTincHost";
        module = "tinc";
        controller = "settings"
    }
    switch ("Enable-OPNsenseTincHost".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "tinc" "Enable-OPNsenseTincHost" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Enable-OPNsenseTincHost -Uuid $uuid } | should Not Throw
            }
