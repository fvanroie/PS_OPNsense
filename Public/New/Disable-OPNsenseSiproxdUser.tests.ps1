...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseSiproxdUser";
        module = "siproxd";
        controller = "user"
    }
    switch ("Disable-OPNsenseSiproxdUser".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "siproxd" "Disable-OPNsenseSiproxdUser" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseSiproxdUser -Uuid $uuid } | should Not Throw
            }
