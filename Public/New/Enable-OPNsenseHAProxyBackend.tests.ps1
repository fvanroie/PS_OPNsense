...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Enable-OPNsenseHAProxyBackend";
        module = "haproxy";
        controller = "settings"
    }
    switch ("Enable-OPNsenseHAProxyBackend".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "haproxy" "Enable-OPNsenseHAProxyBackend" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Enable-OPNsenseHAProxyBackend -Uuid $uuid } | should Not Throw
            }
