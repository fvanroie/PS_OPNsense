...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseTorHiddenServiceAuth";
        module = "tor";
        controller = "general"
    }
    switch ("Disable-OPNsenseTorHiddenServiceAuth".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "tor" "Disable-OPNsenseTorHiddenServiceAuth" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseTorHiddenServiceAuth -Uuid $uuid } | should Not Throw
            }
