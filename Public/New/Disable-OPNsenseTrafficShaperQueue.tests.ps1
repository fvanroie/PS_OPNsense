...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseTrafficShaperQueue";
        module = "trafficshaper";
        controller = "settings"
    }
    switch ("Disable-OPNsenseTrafficShaperQueue".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    switch ("trafficshaper") {
        { @("AcmeClient", "Freeradius", "IDS", "Monit", "Postfix", "ProxyUserACL", "Quagga", "Routes", "Siproxd", "Tinc", "Tor", "Zerotier") -Contains $_ } {           
            $objects.add('command',"searchqueue")
        }
        default {           
            $objects.add('command',"searchqueues")
        }
    }
    ...
    Describe -Tag "trafficshaper" "Disable-OPNsenseTrafficShaperQueue" {
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseTrafficShaperQueue -Uuid $uuid } | should Not Throw
            }
