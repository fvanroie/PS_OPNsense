...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Enable-OPNsenseIDSUserRule";
        module = "ids";
        controller = "settings"
    }
    switch ("Enable-OPNsenseIDSUserRule".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "ids" "Enable-OPNsenseIDSUserRule" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Enable-OPNsenseIDSUserRule -Uuid $uuid } | should Not Throw
            }
