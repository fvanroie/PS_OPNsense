...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Enable-OPNsenseFreeradiusClient";
        module = "freeradius";
        controller = "client"
    }
    switch ("Enable-OPNsenseFreeradiusClient".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "freeradius" "Enable-OPNsenseFreeradiusClient" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Enable-OPNsenseFreeradiusClient -Uuid $uuid } | should Not Throw
            }
