...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseRoutesroute";
        module = "routes";
        controller = "routes"
    }
    switch ("Disable-OPNsenseRoutesroute".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    switch ("routes") {
        { @("AcmeClient", "Freeradius", "IDS", "Monit", "Postfix", "ProxyUserACL", "Quagga", "Routes", "Siproxd", "Tinc", "Tor", "Zerotier") -Contains $_ } {           
            $objects.add('command',"searchroute")
        }
        default {           
            $objects.add('command',"searchroutes")
        }
    }

    Describe -Tag "routes" "Disable-OPNsenseRoutesroute" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseRoutesroute -Uuid $uuid } | should Not Throw
            }
