...
InModuleScope PS_OPNsense {
    ...
    switch ("Disable-OPNsenseCronJob".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    switch ("cron") {
        { @("AcmeClient", "Freeradius", "IDS", "Monit", "Postfix", "ProxyUserACL", "Quagga", "Routes", "Siproxd", "Tinc", "Tor", "Zerotier") -Contains $_ } {           
            $objects.add('command',"searchjob")
        }
        default {           
            $objects.add('command',"searchjobs")
        }
    }

    Describe -Tag "cron" "Disable-OPNsenseCronJob" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseCronJob -Uuid $uuid } | should Not Throw
            }
