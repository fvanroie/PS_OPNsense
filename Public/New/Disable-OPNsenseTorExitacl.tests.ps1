...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseTorExitacl";
        module = "tor";
        controller = "exitacl"
    }
    switch ("Disable-OPNsenseTorExitacl".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "tor" "Disable-OPNsenseTorExitacl" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseTorExitacl -Uuid $uuid } | should Not Throw
            }
