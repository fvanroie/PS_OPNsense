...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseIDSUserRule";
        module = "ids";
        controller = "settings"
    }
    switch ("Disable-OPNsenseIDSUserRule".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    switch ("ids") {
        { @("AcmeClient", "Freeradius", "IDS", "Monit", "Postfix", "ProxyUserACL", "Quagga", "Routes", "Siproxd", "Tinc", "Tor", "Zerotier") -Contains $_ } {           
            $objects.add('command',"searchuserrule")
        }
        default {           
            $objects.add('command',"searchuserrules")
        }
    }

    Describe -Tag "ids" "Disable-OPNsenseIDSUserRule" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseIDSUserRule -Uuid $uuid } | should Not Throw
            }
