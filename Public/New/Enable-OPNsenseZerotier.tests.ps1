...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Enable-OPNsenseZerotier";
        module = "zerotier";
        controller = "network"
    }
    switch ("Enable-OPNsenseZerotier".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "zerotier" "Enable-OPNsenseZerotier" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Enable-OPNsenseZerotier -Uuid $uuid } | should Not Throw
            }
