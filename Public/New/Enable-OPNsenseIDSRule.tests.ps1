...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Enable-OPNsenseIDSRule";
        module = "ids";
        controller = "settings"
    }
    switch ("Enable-OPNsenseIDSRule".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "ids" "Enable-OPNsenseIDSRule" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Enable-OPNsenseIDSRule -Uuid $uuid } | should Not Throw
            }
