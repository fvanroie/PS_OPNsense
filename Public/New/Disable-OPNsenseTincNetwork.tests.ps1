...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseTincNetwork";
        module = "tinc";
        controller = "settings"
    }
    switch ("Disable-OPNsenseTincNetwork".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "tinc" "Disable-OPNsenseTincNetwork" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseTincNetwork -Uuid $uuid } | should Not Throw
            }
