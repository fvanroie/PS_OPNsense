...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Enable-OPNsenseTrafficShaperPipe";
        module = "trafficshaper";
        controller = "settings"
    }
    switch ("Enable-OPNsenseTrafficShaperPipe".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "trafficshaper" "Enable-OPNsenseTrafficShaperPipe" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Enable-OPNsenseTrafficShaperPipe -Uuid $uuid } | should Not Throw
            }
