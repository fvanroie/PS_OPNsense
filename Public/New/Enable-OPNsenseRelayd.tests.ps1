...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Enable-OPNsenseRelayd";
        module = "relayd";
        controller = "status"
    }
    switch ("Enable-OPNsenseRelayd".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "relayd" "Enable-OPNsenseRelayd" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Enable-OPNsenseRelayd -Uuid $uuid } | should Not Throw
            }
