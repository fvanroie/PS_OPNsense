...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseOspf6Interface";
        module = "quagga";
        controller = "ospf6settings"
    }
    switch ("Disable-OPNsenseOspf6Interface".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "quagga" "Disable-OPNsenseOspf6Interface" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseOspf6Interface -Uuid $uuid } | should Not Throw
            }
