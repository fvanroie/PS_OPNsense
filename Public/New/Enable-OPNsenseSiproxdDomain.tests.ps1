...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Enable-OPNsenseSiproxdDomain";
        module = "siproxd";
        controller = "domain"
    }
    switch ("Enable-OPNsenseSiproxdDomain".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "siproxd" "Enable-OPNsenseSiproxdDomain" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Enable-OPNsenseSiproxdDomain -Uuid $uuid } | should Not Throw
            }
