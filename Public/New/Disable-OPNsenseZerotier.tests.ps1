...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Disable-OPNsenseZerotier";
        module = "zerotier";
        controller = "network"
    }
    switch ("Disable-OPNsenseZerotier".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "zerotier" "Disable-OPNsenseZerotier" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Disable-OPNsenseZerotier -Uuid $uuid } | should Not Throw
            }
