...
InModuleScope PS_OPNsense {
    $objects = @{
        function = "Enable-OPNsenseRoutesroute";
        module = "routes";
        controller = "routes"
    }
    switch ("Enable-OPNsenseRoutesroute".Substring(0,6)) {
        "Enable"  {
            $objects.add('state','1')
        }
        default {
    ...
    Describe -Tag "routes" "Enable-OPNsenseRoutesroute" {
        $uuid = @()
        ...
        Context "Get UUIDs" {
            ...
            It "<module> : <function>" -TestCases $objects {
                param($function, $module, $controller, $command, $state)
                { Enable-OPNsenseRoutesroute -Uuid $uuid } | should Not Throw
            }
